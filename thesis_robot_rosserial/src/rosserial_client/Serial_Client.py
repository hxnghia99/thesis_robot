...
class Publisher:
    ...
    """
    #check error from topic_info when creating publisher
    def __init__(self, topic_info):
        ...
        if self.message._md5sum == topic_info.md5sum:
            self.publisher = rospy.Publisher(self.topic, self.message, queue_size=20)
        else:
            raise Exception('Checksum does not match: ' + self.message._md5sum + ',' + topic_info.md5sum)
    #publish data with ros_msgs type
...
#driver for communicating between ROS master (client) and STM32F4 (server)
class SerialClient(object):
    ...
    def __init__(self, port='/dev/ttyUSB0', baud=115200, timeout=5.0, fix_pyserial_for_test=False):
    ...
    #send Tx start request to server
    ...
    #Send Tx stop request to server
    ...
    #Read <length> data of bytes from port usb
    ...
    #Check data after send Tx start request 
    def run(self):
        # Launch write thread.
        ...
        while self.write_thread.is_alive() and not rospy.is_shutdown():
            if (rospy.Time.now() - self.lastsync).to_sec() > (self.timeout * 3):
                if self.synced:
                    ...
                    rospy.logerr("Unable to sync with device; possible link problem or link software version mismatch")
                self.lastsync_lost = rospy.Time.now()
                self.sendDiagnostics(diagnostic_msgs.msg.DiagnosticStatus.ERROR, ERROR_NO_SYNC)
                self.requestTopics()
                self.lastsync = rospy.Time.now()
            
            #If not timeout then start 
            try:
                with self.read_lock:
                ...
                # Read message length 2 bytes, checksum (3 bytes)
                ...
                # reinitialize their topics if having error
                with self.read_lock:
                    self.port.flushInput()
    ...
    def sendDiagnostics(self, level, msg_text):
        ...
        status.name = "thesis_robot_rosserial"
